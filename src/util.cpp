...
const char * const BITCOIN_CONF_FILENAME = "xeonchain.conf";
const char * const BITCOIN_PID_FILENAME = "xeonchain.pid";
...
#else
    const char* pszModule = "xeonchain";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XeonChain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XeonChain";
#else
    // Unix
    return pathRet / ".xeonchain";
#endif
#endif
}
